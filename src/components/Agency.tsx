...
import { allUsers, editUser, updateUser, CreateUser, deleteUser, getAgency, updateAgency, editAgency, CreateAgency, deleteAgency } from '../api/auth';
...
//Generate page numbers with ellipses
  ...
  const handleSubmit = async (e: any) => {
    e.preventDefault();
     setisLoader(true);
      try {
              const res = await updateAgency(selectedUser.id, selectedUser.name, 
                selectedUser.email, selectedUser.oto_1, selectedUser.password);
  ...
  const handleEdit = async (userId: string) => {
    try {
      const res = await editAgency(userId);
...
const handleAddUser = async (e: React.FormEvent) => {
  ...
  const handleDelete = async (userId: string) => {
    try {
    const res = await deleteAgency(userId);
    Swal.fire({
      toast: true,
      icon: "success",
      title: res.message,
